...
impl ResponseError for TaskError {
    ...
    fn status_code(&self) -> StatusCode {
        match self {
            ...
            TaskError::TaskCreationFailure => StatusCode::BAD_REQUEST,
...
pub async fn submit_task(
    ddb_repo: Data<DDBRepository>,
    task_request: Json<SubmitTaskRequest>,
) -> Result<Json<TaskIdentifier>, TaskError> {
    let task = Task::new(
        task_request.user_uiid.clone(),
    ...
    match ddb_repo.put_task(task).await{
        Ok(_) => Ok(Json(TaskIdentifier{task_global_id})),
